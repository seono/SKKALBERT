...
from src.korquad_utils import read_squad_examples
...
def train(args, train_dataset, model, tokenizer, result_list=None):
    ...
    for epoch in train_iterator:
        epoch_iterator = tqdm(
            train_dataloader, desc="Train(XX Epoch) Step(X/X) (loss=X.X)", leave=True, position=0
        )
        ...
        with open(output_eval_file, "a", encoding='utf-8') as f:
            results = evaluate(args, model, tokenizer, prefix=epoch, global_step=global_step)
            f.write("{} epoch\n\n".format(epoch))
...
def evaluate(args, model, tokenizer, prefix="", global_step=None):
    ...
    epoch_iterator = tqdm(eval_dataloader, leave=True, position=0)
    ...
    # Compute predictions
    output_prediction_file = os.path.join(args.output_dir, "predictions_{}_{}.json".format(prefix, args.eda_type))
    output_nbest_file = os.path.join(args.output_dir, "nbest_predictions_{}_{}.json".format(prefix, args.eda_type))
    ...
    # output_eval_file = os.path.join(output_dir, "eval_result_{}_{}_{}.txt".format(list(filter(None, args.model_name_or_path.split("/"))).pop(),
    #                                                                            args.eda_type,global_step))
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file):
        ...
        if evaluate is False:
            ...
            examples = read_squad_examples(input_file=args.train_file, is_training=False, version_2_with_negative=False, args=args)
...
def main(cli_args):
    ...
    args.eda_type=cli_args.eda_type
    args.alpha=cli_args.alpha
    args.num_aug=cli_args.num_aug
    args.eda_model_name=cli_args.eda_model_name
    args.min_score=cli_args.min_score
    args.dict_file_name=cli_args.dict_file_name
    if cli_args.test is True:
        args.train_file="data/korquad/sample.json"
...
if __name__ == "__main__":
    ...
    cli_parser.add_argument("--test", default=False, type=bool)
    cli_parser.add_argument("--eda_type", default="no_eda", type=str, help="eda type e.g) sr, ri, rs, rd")
    cli_parser.add_argument("--alpha", default=0.1, type=float, help="For eda")
    cli_parser.add_argument("--num_aug", default=1, type=int, help="For eda")
    cli_parser.add_argument("--eda_model_name", default="word2vec", type=str, help="For eda")
    cli_parser.add_argument("--min_score", default=0.7, type=float, help="For eda")
    cli_parser.add_argument("--dict_file_name", default="data/syn/synonyms_file.pickle", type=str)
    cli_parser.add_argument("--do_list", default=False, type=bool)
