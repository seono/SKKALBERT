...
def get_synonyms(word, min_score=0.75):
    synonyms = set()
...
def test_eda(sentence):
    ...
    a_words = synonym_replacement(sentence, n, 0.75)
    augmented_sentences.append(' '.join(a_words))
    a_words = random_insertion_ver2(words, n, 0.75)
    augmented_sentences.append(' '.join(a_words))
...
def sentence_rd(sentences,except_list, n, num_aug):
    ...
    while len(contexts)<num_aug-1:
        ...
        for i in range(len(sentences)):
            ...
            pop_list.append(i)
        new_sentences = sentences[:]
        pop_list = random.sample(pop_list,n)
...
def sentence_eda(context, orig_answer_text, eda_type, alpha=0.1, num_aug=2):
    ...
    p = 0
    except_list=set()
    for i,st in enumerate(new_sentences):
        p+=len(st)
        p+=1
        if answer_start<p:
            except_list.add(i)
    ...
    for i,st in enumerate(new_sentences):
        ...
        if answer_end<p:
            except_list.add(i)
            break
    n = max(1,int(alpha*len(new_sentences)))
    if eda_type=="st_rs":
