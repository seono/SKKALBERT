...
from .remove_tag import preprocess_text
...
def build_parser():
    ...
    parser.add_argument("--data_path", dest="data_path", default='train/', type=str, help='전환하려는 데이터가 들어있는 폴더')
    parser.add_argument("--save_path", dest="save_path", default='train/modified/', type=str, help='전환후 데이터가 저장될 폴더')
    ...
    parser.add_argument("--original_qas_include", action="store_true", default=False,
    ...
    config.worker = cpu_count()
    return config


def merge_file(file, merged_files, version):
    if merged_files is None:
        merged_files = {'version': version, 'data': []}
...
def convert_file_into_squad(config):
    ...
    config.worker = cpu_count()
    version = ""
    print("multiprocess cpu_count : {}".format(config.worker))
    if config.worker > 1:
        ...
        with open(file_paths[0],'r',encoding='utf-8') as open_file:
            version = json.load(open_file, object_pairs_hook=OrderedDict)['version']
        with Pool(processes=config.worker) as p:
            with tqdm(total=len(file_names), desc='Converting_File', unit='file') as pbar:
                for i, modified_file in enumerate(
                    ...
                    merged_files = merge_file(modified_file, merged_files, version)
                    print("{} merged".format(file_paths[i]))
                    pbar.update()
    else:
...
def convert_data_into_squad(args):
    ...
    for data in data_list:
        modified_paragraphs = converter.convert_to_squad_format(data['context'],data['qas'])
        title = data['title']
    ...
    def get_modified_paragraph(self, paragraph, paragraph_id, qas):
        ...
        for qa in qas:
            ...
            if len(possible_ids) > 0 and possible_ids is not None:
                ...
                paragraph, modified_start, modified_end = preprocess_text(paragraph, modified_start, modified_end)
