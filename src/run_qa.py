...
def train(args, train_dataset, model, _):
    """ Train the model """
...
def load_and_cache_examples(args, tokenizer, data, input_file):
    # Load data features from cache or dataset file
    # one input file, change to diretory
    ver = 1
    logger.info("korquad_utils version %d" %ver)
    logger.info("Creating features from dataset file at %s", input_file)
    examples = read_squad_examples(data=data, is_training=True, version_2_with_negative=False)
    features = convert_examples_to_features(examples=examples,
                                            ...
                                            is_training=True)
    logger.info("End convert_examples_to_features")
...
def main():
    ...
    epochs = 0
    if args.extract_exam is True:
        logger.info("Parse and extract example")
        for a in range(args.train_file_start,args.train_file_end+1):
            if a<10: a='0'+str(a)
            else : a=str(a)
            open_file=args.train_file_name+a+'.json'
            with open(open_file, encoding="utf-8") as data_file:
                datas = json.load(data_file, object_pairs_hook=OrderedDict)
            i = 0
            if args.extract_start>0:
                i=args.extract_start
                args.extract_start = 0
            while (i+1)*parse_size <= len(datas["data"]):
                data=datas["data"][i*parse_size:(i+1)*parse_size]
                logger.info("file num : %s, parse %d to %d", a, i*parse_size, (i+1)*parse_size)
                train_dataset = load_and_cache_examples(args, tokenizer, data, open_file)
                save_examples(args,train_dataset,a,str(i))
                i+=1

    
    if args.load_examples is True:
